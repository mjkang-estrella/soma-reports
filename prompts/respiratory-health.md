# Respiratory Health local-agent prompt

Generate a plain-English Respiratory Health report from local genome-derived evidence and supplied reference resources.
...
For this package, focus on respiratory health education, asthma/COPD/CF and infection-risk limits, spirometry, oxygen, exercise-clearance, pediatric respiratory, inhaler, medication, and treatment claim boundaries.
If validated respiratory model, spirometry, oxygen saturation, pulmonary diagnosis, asthma or COPD context, cystic fibrosis context, infection history, symptoms, imaging, medication list, exercise tolerance, pediatric context, clinician review, and authenticated sample-report rows are missing, mark those sections unavailable instead of inferring them.
